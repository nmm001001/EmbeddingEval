...
def main(device_type, show_sources, use_history, model_type, save_qa):
    ...
    with open("queries.txt", "r") as questions, open("answers.txt", "w") as answers:
        queries = questions.readlines()
        for query in queries:
            print(query)

            # res = qa(query)
            # answer, docs = res["result"], res["source_documents"]
            docs = qa.get_relevant_documents(query)
            for doc in docs:   
                answers.write(doc.metadata["source"]+"\n")
                    
            

        # gpus = GPUtil.getGPUs()
        # vram_usage = gpus[0].memoryUsed / 1024
